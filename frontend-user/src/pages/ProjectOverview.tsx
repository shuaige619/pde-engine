...
export default function ProjectOverview() {
  ...
  const progress = Math.round(pipeline?.progress ?? currentProject.progress ?? 0);
  const artifactCount = currentProject._count?.artifacts ?? 0;

  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={8}><Card><Statistic title="当前阶段" value={currentProject.currentStage || '未开始'} /></Card></Col>
        <Col span={8}><Card><Statistic title="整体进度" value={`${progress}%`} /></Card></Col>
        <Col span={8}><Card><Statistic title="产物数量" value={artifactCount} /></Card></Col>
      </Row>
      <Card title="流程阶段">
          ...
          <>
            ...
            <Timeline
              ...
              items={stageNames.map((name, i) => ({
                label: `阶段 ${i + 1}`,
                children: name,
                color: i < Math.floor(progress / 8.3) ? 'green' : 'gray',
              }))}
            />
          </>
        )}
